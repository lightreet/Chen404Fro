...
- `npm test` 统一运行认证、音乐播放与歌词解析、精确 ID、手机导航、旅行上传和音乐管理结构回归。原有两条固定宽度及分页贴底 CSS 快照已不符合确认后的界面，移除后由真实浏览器检查覆盖视觉布局；构建检查仍不替代测试和页面验收。
...
- 前后端扫描与缺陷台账：[`doc/前后端代码与文档扫描报告_2026-08-14.md`](./doc/前后端代码与文档扫描报告_2026-08-14.md)
